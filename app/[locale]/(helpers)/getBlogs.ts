...
function mapRowToPost(row: BlogRow, locale: Locale): BlogPost {
  const title = locale === 'en' ? row.title_english : locale === 'pt' ? row.title_portuguese : row.title_spanish;
  const excerpt = locale === 'en' ? row.excerpt_english : locale === 'pt' ? row.excerpt_portuguese : row.excerpt_spanish;
  const content = locale === 'en' ? row.content_english : locale === 'pt' ? row.content_portuguese : row.content_spanish;
  const metaTitle = locale === 'en' ? row.meta_title_english : locale === 'pt' ? row.meta_title_portuguese : row.meta_title_spanish;
  const metaDescription = locale === 'en' ? row.meta_description_english : locale === 'pt' ? row.meta_description_portuguese : row.meta_description_spanish;
  const category = locale === 'en' ? row.category_english : locale === 'pt' ? row.category_portuguese : row.category_spanish;
  const tagsRaw = locale === 'en' ? row.tags_english : locale === 'pt' ? row.tags_portuguese : row.tags_spanish;
  const tags = tagsRaw && typeof tagsRaw === 'string' ? tagsRaw.split(',').map(t => t.trim()).filter(Boolean) : [];
    ...
    if (parsedLinks) {
      ...
      if (Array.isArray(parsedLinks.custom)) {
        for (const customLink of parsedLinks.custom) {
          if (customLink && typeof customLink.url === 'string' && customLink.url.trim()) {
            socialLinks.push({
              platform: 'custom',
              url: customLink.url,
